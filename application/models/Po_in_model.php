...
class po_in_model extends CI_Model {
 
    var $table = 'po_in';
    var $column_order = array(null, "po_in.no_trans", "po_in.tgl_po", "pelanggan.nama_pelanggan",null, "pembayaran.nama_pembayaran", "user.nama", "po_in.status", null); //set column field database for datatable orderable
    var $column_search = array('po_in.no_trans'); //set column field database for datatable searchable just firstname , lastname , address are searchable
    var $order = array('po_in.tgl_po' => 'desc'); // default order 
    ...
    public function laporan_tgl($src_po, $src_from, $src_to, $src_supplier, $src_user, $src_status, $src_bayar){
        ...
        $this->db->join("user", "user.username = po_in.created_by");
        ...
        $this->db->order_by("po_in.created_on", "ASC");
