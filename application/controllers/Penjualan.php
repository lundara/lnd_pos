...
class penjualan extends CI_Controller {
    ...
    function print_inv(){
        ...
        $this->db->where("penjualan.no_trans", $id);
        ...
        $this->db->join("produk", "produk.id = penjualan_detail.iditem");
        $this->db->where("penjualan_detail.no_trans", $id);
    ...
    public function add_draft(){
        ...
        $q = $this->db->get("penjualan_detail");
        $d = $q->row_array();
        $n = $q->num_rows();
        ...
        $this->db->trans_start();
            ...
            if ($n == 0) {
                ...
                $this->db->where("no_trans", $no_trans);
                $this->db->where("iditem", $id);
                $this->db->update("penjualan_detail", $dt);
            }
    ...
    function draft(){
        ...
        $this->db->where("no_trans", $no_trans);
        $c = $this->db->get("penjualan")->num_rows();
        ...
        $this->db->trans_start();
            ...
            for($i=1;$i<=$session_no;$i++){
                if ($_SESSION['pjp'][$i][2]!="") {
                    ...
                    $this->db->insert("penjualan_detail", $dt2);
    ...
    function add_pembayaran(){
        ...
        $this->db->trans_start();
            ...
            $this->db->where("no_trans", $no_trans);
            $this->db->update("penjualan", $dt);

            $this->db->insert("pembayaran", $dt2);
    ...
    function save(){
        ...
        $this->db->trans_start();
            ...
            $this->db->insert("pembayaran", $dt_bayar);
            ...
            for($i=1;$i<=$session_no;$i++){
                if ($_SESSION['pjp'][$i][2]!="") {
                    ...
                    if ($cks == 0) {
                        ...
                        $this->db->order_by("created_on", "desc");
                        $this->db->limit(1);
                        $ks = $this->db->get("stok_log")->row_array();
                        $dtks = array(
    ...
    function save_draft(){
        ...
        $this->db->where("no_trans", $no_trans);
        $det = $this->db->get("penjualan_detail")->result();
        ...
        $this->db->trans_start();
            ...
            foreach ($det as $vdet) {
                ...
                $this->db->where("no_trans", $no_trans);
                $dn = $this->db->get("penjualan")->row_array();
    ...
    public function data_item(){
        ...
        for($i=1;$i<=$session_no;$i++){
            ...
            if ($_SESSION['pjp'][$i][2]!="") {
                ...
                $this->db->where("produk.id", $_SESSION['pjp'][$i][0]);
    ...
    function pembayaran(){
        ...
        $this->db->join("user", "user.username = pembayaran.created_by");
        $this->db->where("pembayaran.no_trans", $id);
        $d = $this->db->get()->result();

        $slb = "
            pembayaran.*,
            SUM(pembayaran.nominal) AS bayar
        ";
        $this->db->select($slb);
        $this->db->from("pembayaran");
        $this->db->where("pembayaran.no_trans", $id);
       ...
       $slp = "
            ...
            SUM( (penjualan_detail.harga * penjualan_detail.qty) - (penjualan_detail.harga * penjualan_detail.qty)*penjualan_detail.disc/100  ) AS piutang,
            penjualan.no_trans,
            penjualan.lunas,
            penjualan.status,
        ";
        ...
        foreach ($d as $v) {
            ...
            $dt = array(
                    ...
                    "piutang"   => number_format($dtp["piutang"], 0, ",", "."),
                    "sisa"      => $sisa
                );
