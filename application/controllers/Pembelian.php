...
class pembelian extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->load->model('Pembelian_model','md');
	...
	public function index(){        
        ...
        $this->lnd->akses("pembelian", "submenu");
        ...
        //$ambildata = $this->mread->export_kontak();
            ...
            foreach ($list as $v){
               ...
               //pemanggilan sesuaikan dengan nama kolom tabel
                ...
                $this->db->join("produk", "produk.id = pembelian_detail.iditem");
                $this->db->join("satuan", "satuan.id = produk.idsatuan");
                $this->db->join("pembelian", "pembelian.no_trans = pembelian_detail.no_trans");
            ...
            foreach ($list as $v){
               ...
               //pemanggilan sesuaikan dengan nama kolom tabel
                ...
                $this->db->join("produk", "produk.id = pitstop_penjualan_detail.iditem");
                $this->db->join("satuan", "satuan.id = produk.idsatuan");
    ...
    public function add(){
        ...
        $this->db->where("id", $id);
        $d = $this->db->get("produk")->row_array();
        ...
        $_SESSION['pit_pmb'][$sess_id][4]=""; //satuan
    ...
    function save(){
        ...
        $c = $this->db->get("pembelian")->num_rows();
        ...
        $dt_act = array(
            "deskripsi"         => $user["nama"]." telah menambah data Pembelian ".$no_trans.".",
        ...
        $this->db->trans_start();
            ...
            for($i=1;$i<=$session_no;$i++){
                if ($_SESSION['pit_pmb'][$i][2]!="") {
                	...
                	//$this->db->update("produk", $dt_modal);
                    ...
                    if ($cks == 0) {
                        ...
                        $this->db->order_by("created_on", "desc");
                        $this->db->limit(1);
                        $ks = $this->db->get("kartu_stok")->row_array();
                        $dtks = array(
                        ...
                        $this->db->insert("kartu_stok", $dtks);
                    }
                    else{
                        $dtks = array(
                                "idproduk"  => $_SESSION['pit_pmb'][$i][0],
                        ...
                        $this->db->update("kartu_stok", $dtks);
                    }
                    ...
                    $this->db->insert("pembelian_detail", $dt2);
    ...
    public function data_item(){
        ...
        for($i=1;$i<=$session_no;$i++){
            ...
            if ($_SESSION['pit_pmb'][$i][2]!="") {
                ...
                $this->db->join("satuan", "satuan.id = produk.idsatuan");
                $this->db->where("produk.id", $_SESSION['pit_pmb'][$i][0]);
    ...
    function get_trans(){
        ...
        $d = json_encode(array(
                ...
                "no_trans"  =>"LNDB-".date("ym").sprintf("%03d", $digit+1)
            ));
    ...
    function detail(){
        ...
        $this->db->join("satuan", "satuan.id = produk.idsatuan");
        $this->db->join("pembelian", "pembelian.no_trans = pembelian_detail.no_trans");
        $this->db->where("pembelian_detail.no_trans", $id);
        $d = $this->db->get()->result();
    ...
    public function data(){
            ...
            foreach ($list as $v) {
                ...
                if ($total["ppn"] == "Y") {
                    $gt = $total['total'] + ($total['total']*10/100) ;
                }
                else{
                    $gt = $total['total'];
                }
