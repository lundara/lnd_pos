...
class po_in extends CI_Controller {

    function __construct(){
        parent::__construct();
        date_default_timezone_set("Asia/Jakarta");
        $this->load->model('po_in_model','md');
    ...
    function select2(){
        ...
        $this->db->like("po_in.no_po", $_POST["term"]);
        $this->db->where("po_in.status", "Belum Lunas");
        ...
        foreach ($d as $v) {
            ...
            if ($v->jatuh_tempo=="0000-00-00") {
                $tempo = "";
            }
            else{
                $tempo = date("d/m/Y", strtotime($v->jatuh_tempo));
            }
            ...
            $slb = "
                ...
                po_in.no_trans,
                po_in.no_po,
                SUM(invoice_out.nominal) AS bayar
            ";
            $this->db->select($slb);
            $this->db->from("invoice_out");
            $this->db->join("po_in", "invoice_out.no_po = po_in.no_po");
            $this->db->where("invoice_out.no_po", $v->no_po);
            ...
            $ttl = 0;
            foreach ($det as $vdet) {
                $ttl = $ttl+ ($vdet->harga*$vdet->qty);
            }
            $ppn = ($ttl*10/100);
            $dt = array(
                    "id"            => $v->no_trans,
                    "no_po"         => $v->no_po,
                    "tgl_po"        => date("d/m/Y", strtotime($v->tgl_po)),
                    "jatuh_tempo"   => $tempo,
                    "pelanggan"     => $v->nama_pelanggan,
                    "idpembayaran"  => $v->nama_pembayaran,
                    "ppn"           => $v->ppn,
                    "bayar"         => "<span style='color:red'>".number_format($bayar["bayar"], 0, ",", ".")."</span>",
                    "ttl"           => number_format($ttl + $ppn, 0, ",", "."),
                    "sisa"          => number_format(($ttl + $ppn) - $bayar["bayar"], 0, ",", ".")
                );
    ...
    function excel(){
        //$ambildata = $this->mread->export_kontak();
            ...
            $objget->setCellValueByColumnAndRow(0, 1, "LAPORAN PO IN");            
            ...
            foreach ($list as $v){
               ...
               //pemanggilan sesuaikan dengan nama kolom tabel
                ...
                $objset->setCellValue("B".$baris, date("d/m/Y", strtotime($v->tgl_po)) );
                ...
                foreach ($det as $vdet) {
                    ...
                    if ($vdet->delivery_date=="0000-00-00") {
                        $dd = "";
                    }
                    else{
                        $dd = date("d/m/Y", strtotime($vdet->delivery_date));
                    }

                    $objset->setCellValue("F".$baris2, $vdet->qty );
                    $objset->setCellValue("G".$baris2, $dd );
            ...
            $objset->setCellValue("N".$baris, $ak_ppn+$ak_total);
            ...
            $filename = urlencode("Laporan PO In.xlsx");
    ...
    function save(){
        ...
        $dt_act = array(
            "deskripsi"         => $user["nama"]." telah menambah data PO IN ".$no_trans.".",
        ...
        $this->db->trans_start();
            ...
            $this->db->insert("po_in", $dt);

            $this->db->where("no_po", $no_po);
            $idp = $this->db->get("po_in")->row_array();

            $session_no = count($_SESSION['poin']);
            for($i=1;$i<=$session_no;$i++){
                if ($_SESSION['poin'][$i][2]!="") {

                    $ppn = ($_SESSION['poin'][$i][3]) - (($_SESSION['poin'][$i][3]) * 10 / 100);
                    $tax = ($_SESSION['poin'][$i][3]) - $ppn;


                    $dt2 = array(
                            "iditem"        => $_SESSION['poin'][$i][0],
                            ...
                            "delivery_date" => $this->lnd->toTglSys($_SESSION['poin'][$i][9])
                        );

                    $this->db->insert("po_in_detail", $dt2);
    ...
    public function data(){
            ...
            foreach ($list as $v) {
                ...
                $sl = "
                    ...
                    po_in_detail.qty,
                    po_in_detail.disc,
                    SUM( (po_in_detail.harga * po_in_detail.qty) - ( (po_in_detail.harga * po_in_detail.qty) * po_in_detail.disc/100) ) AS total
                ";
                $this->db->select($sl);
                $this->db->from('po_in_detail');
                $this->db->where("po_in_detail.no_trans", $v->no_trans);
                ...
                if ($v->lampiran!="") {
                    $lmp = "<i class='fa fa-paperclip fa-lg'></i>";
                }
                else{
                    $lmp = "";
                }
                ...
                $row[] = $v->no_po." ".$lmp;
                $row[] = "<center>".date("d/m/Y", strtotime($v->tgl_po))."</center>";
                ...
                $row[] = "
                            ...
                            <li class='btn-group dropdown act' >
                                ...
                                <ul class='dropdown-menu' id='dropdown-menu".$no."'  role='menu' style='width:auto;min-width:2px'>
                                    ...
                                    <li>
                                        <a href='javascript:;' onclick='lampiran(\"".$v->no_trans."\")' data-toggle='modal' data-target='#md-po'>
                                            <i class='fa fa-paperclip fa-lg'></i> Lampiran
                                        </a>
                                    </li>
