...
class po_out extends CI_Controller {
    ...
    function excel(){
        //$ambildata = $this->mread->export_kontak();
            ...
            $listu = $this->md->laporan_user($src_po, $src_from, $src_to, $src_supplier, $src_user, $src_status);
            $nou = 1;
            $ak_totalu= 0;
            $ak_ppnu   = 0;
            $ak_gtu    = 0;
            ...
            foreach ($listu as $vu){
               ...
               //pemanggilan sesuaikan dengan nama kolom tabel
                ...
                foreach ($detu as $vdetu) {
                    ...
                    $sheet2->setCellValue("G".$baris2u, $vdetu->qty );
    ...
    public function add(){
        ...
        $session_no = count($_SESSION['poout']);
        ...
        if ($session_no!=0) {
            for($i=1;$i<=$session_no;$i++){
                ...
                if ($_SESSION['poout'][$i][0] == $id) {
                    $mqty       = $qty + $_SESSION['poout'][$i][1];
                    $sess_id    = $_SESSION['poout'][$i][2];
                    break 1;
                }
                else{
                    $mqty = $qty;
        ...
        $_SESSION['poout'][$sess_id][0]=$id; // idproduk
        $_SESSION['poout'][$sess_id][1]=$mqty; //qty
        $_SESSION['poout'][$sess_id][2]=$sess_id; //session id
        $_SESSION['poout'][$sess_id][3]=$harga; //harga
        $_SESSION['poout'][$sess_id][4]=$satuan; //satuan
        $_SESSION['poout'][$sess_id][5]=$lead_time;
        $_SESSION['poout'][$sess_id][6]=$modal;
        $_SESSION['poout'][$sess_id][7]=$ket;
        $_SESSION['poout'][$sess_id][8]=$disc;
        $_SESSION['poout'][$sess_id][9]=$estimasi;
    ...
    public function del(){
        ...
        unset($_SESSION['poout'][$id][0]);
        unset($_SESSION['poout'][$id][1]);
        unset($_SESSION['poout'][$id][2]);
        unset($_SESSION['poout'][$id][3]);
        unset($_SESSION['poout'][$id][4]);
        unset($_SESSION['poout'][$id][5]);
        unset($_SESSION['poout'][$id][6]);
        unset($_SESSION['poout'][$id][7]);
        unset($_SESSION['poout'][$id][8]);
        unset($_SESSION['poout'][$id][9]);
    ...
    function save(){
        ...
        $dt_act = array(
            "deskripsi"         => $user["nama"]." telah menambah data PO Out ".$no_trans.".",
        ...
        $this->db->trans_start();
            ...
            for($i=1;$i<=$session_no;$i++){
                if ($_SESSION['poout'][$i][2]!="") {

                    $ppn = ($_SESSION['poout'][$i][3]) - (($_SESSION['poout'][$i][3]) * 10 / 100);
                    $tax = ($_SESSION['poout'][$i][3]) - $ppn;

                    if ($_SESSION['poout'][$i][9]!="") {
                       $est = date("Y-m-d", strtotime($_SESSION['poout'][$i][9]));
                    }
                    else{
                        $est = "0000-00-00";
                    }
                    ...
                    $dt2 = array(
                            "iditem"        => $_SESSION['poout'][$i][0],
                            "harga"         => $_SESSION['poout'][$i][3],
                            "tax"           => $tax,
                            "no_trans"      => $no_po,
                            "modal"         => $_SESSION['poout'][$i][6],
                            "qty"           => $_SESSION['poout'][$i][1],
                            "ket"           => $_SESSION['poout'][$i][7],
                            "disc"          => $_SESSION['poout'][$i][8],
                            "estimasi"      => $est
                        );
    ...
    public function des(){
        $this->session->unset_userdata('poout');
    }
    ...
    public function data_item(){
        ...
        for($i=1;$i<=$session_no;$i++){
            ...
            if ($_SESSION['poout'][$i][2]!="") {
                ...
                $this->db->where("produk.id", $_SESSION['poout'][$i][0]);
                ...
                $dt = array(
                        "id_produk"     => $_SESSION['poout'][$i][0],
                        "nama_item"     => $produk["nama_produk"],
                        "qty"           => $_SESSION['poout'][$i][1],
                        "harga"         => $_SESSION['poout'][$i][3],
                        "netto"         => ($_SESSION['poout'][$i][3] * $_SESSION['poout'][$i][1]) - (($_SESSION['poout'][$i][3] * $_SESSION['poout'][$i][1])*$_SESSION['poout'][$i][8]/100) ,
                        "brutto"        => ($_SESSION['poout'][$i][3] * $_SESSION['poout'][$i][1]),
                        "id"            => $_SESSION['poout'][$i][2],
                        "pot"           => "0",
                        "total_item"    => $no,
                        "satuan"        => $_SESSION['poout'][$i][4],
                        "desc"          => $desc,
                        "modal"         => $_SESSION['poout'][$i][6],
                        "lead_time"     => strtoupper($_SESSION['poout'][$i][5]),
                        "ket"           => $_SESSION['poout'][$i][7],
                        "disc"          => $_SESSION['poout'][$i][8],
                        "estimasi"      => $_SESSION['poout'][$i][9]
    ...
    function get_trans(){
        ...
        $d = json_encode(array(
                ...
                "no_trans"  =>"NOBIPO.".date("y").date("m").sprintf("%03d", $digit+1)
            ));
    ...
    function print_po(){
        ...
        foreach ($i as $vi) {
            ...
            $pdf->SetXY(135, $line);
            $pdf->Cell(25,$hd, number_format($vi->harga,2,'.',','),1,1,'R', true);

            $pdf->SetFillColor(210,221,242);
            $pdf->SetXY(160, $line);
            $pdf->Cell(30,$hd, number_format( ($vi->qty*$vi->harga) ,2,'.',','),1,1,'R', true);
        ...
        $pdf->Cell(30,5, number_format($subtotal-$ds,2,'.',','),1,1,'R', true);

        $pdf->SetFillColor(255,255,255);
        $pdf->SetXY(160, $line+15);
        $pdf->Cell(30,5, number_format($tax,2,'.',','),1,1,'R', true);

        $pdf->SetFillColor(210,221,242);
        $pdf->SetXY(160, $line+20);
        $pdf->Cell(30,5, number_format(($subtotal-$ds) +$tax,2,'.',','),1,1,'R', true);
