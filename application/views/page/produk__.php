				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							...
							<select class="form-control select2me" id="varian_title2" onchange="get_varian_detail(this.value)">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select class="form-control select2me" id="varian_detail2">
								<option value="">- Pilih Varian Detail -</option>
							</select>
						</div>
					</div>
					<div class="col-md-12">
						<div class="form-group">
							<button type="submit" class="btn main" id="btn-variant2" onclick="vproduk_add()" style="width: 100%">Tambahkan</button>
						</div>
					</div>
					...
					<div class="col-md-12">
						<div class="table-responsive">
							<table class="table table-bordered">
								<thead>
									<th><center>Varian Group</center></th>
...
<script>
	...
	function edit_varian(id, d, nm){
		...
		$("#btn-varian-batal").show();

	}

	function cancel_edit_varian(){
		$("#btn-varian-batal").hide();
	...
	function vproduk_add(){
		var varian_detail 	= $("#varian_detail2").val();
			...
			$.ajax({
				...
				success:function(data){
					switch(parseFloat(data)){
						case 1:
							//table.ajax.reload(null,false);
							notif("Success", "Data berhasil ditambahkan.", "success");
							//$('#md-variant').modal('toggle');
							get_produk_varian();
						break;
						case 0:
							notif("Error", "Data gagal ditambahkan.", "error");
						break;
						case 0.1:
	...
	function get_varian_detail(id){
		if (id!="") {
			$.ajax({
				url:"<?php echo base_url()?>produk/sel_varian_detail",
				type:"POST",
				dataType:"json",
				data:{id},
				cache:false,
				beforeSend:function(){
					$("#varian_detail2").attr("disabled", "disabled");
					$("#varian_detail2").val("");
					$("#varian_detail2").trigger("change");

				},
				success:function(data){
					var len = data.length;
					var sel = "<option value=''>- Pilih Varian Detail -</option>";

					if (len > 0) {
						for(var i=0;i < len;i++){
							sel+="<option value='"+data[i].id+"'>"+data[i].nama_varian_detail+"</option>";
						}
					}

					$("#varian_detail2").html(sel);
				},
				complete:function(){
					$("#varian_detail2").removeAttr("disabled");
				}
			});
		}
		else{
			$("#varian_detail2").html("<option value=''>- Pilih Varian Detail -</option>");
			$("#varian_detail2").val("");
			$("#varian_detail2").trigger("change");
		...
		if (id!="") {
			$.ajax({
				...
				success:function(data){
					...
					if (len > 0) {
						for(var i=0;i < len;i++){
							var len_detail = data[i]["varian_detail"].length;

							//alert(len_detail);

							for(var o=0;o < len_detail;o++){
								dt+="<tr>";
									if (o == 0) {
										dt+="<td rowspan='"+len_detail+"'>"+data[i].nama_varian+"</td>";
									}
									else{
										dt+="";
									}
									
									dt+="<td>"+data[i]["varian_detail"][o].nama_varian_detail+"</td>";
									dt+="<td align='center'><button class='btn main btn-sm' id='btnv-"+o+"' onclick='vproduk_del(`"+data[i]["varian_detail"][o].id+"`, `"+o+"`)'><i class='fa fa-trash'></i></button></td>";
								dt+="</tr>";
							}
